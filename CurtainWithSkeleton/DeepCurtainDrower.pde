class DeepParticleDrower implements ParticleDrower {
  
  public DeepParticleDrower() {
    println("DeepParticleDrower initialization");
  };
  ...
  void updateInteractions (Particle particle) {
    ...
    if (!particle.pinned) {
      ...
      for (int idx = 0; idx < handPoints.size(); idx++) {
        ...
        float xDistance = curX-width/2 - particle.position.x;
        float yDistance = curY-height/2 - particle.position.y;
        //float zDistance = curZ - particle.position.z;
        float distanceSquared = sq(xDistance) + sq(yDistance);// + sq(zDistance);
        if (distanceSquared < mouseInfluenceSize) { // remember mouseInfluenceSize was squared in setup()
          ...
          ///int zDiff = curZ - prevZ;
          particle.position.add(new PVector(xDiff, yDiff, 0));
          particle.position.z = curZ/2;
        }
/*        if (distanceSquared < mouseInfluenceSize)
          particle.position.sub(new PVector((int)3*xDistance/distanceSquared,(int)3*yDistance/distanceSquared));        
        else        
          particle.position.add(new PVector((int)3*xDistance/distanceSquared,(int)3*yDistance/distanceSquared));
 */         
      }
    }
  }
