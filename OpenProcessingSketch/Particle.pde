...
class Particle {
  ...
  //ArrayList<PVector> multiPositions
   void updateInteractions () {
       mousePressed = true;
/*   if (multiPositions.size()>0){
      System.out.println("Into loop multiPositions");
       Iterator<PVector> itr = multiPositions.iterator();
         while (itr.hasNext()) {
             PVector cur = itr.next();
             mouseX = (int) cur.x;
             mouseY = (int) cur.y;
             
             System.out.println("multiPositions: " + mouseX + ", " + mouseY);
         ...
         System.out.println("Out from  loop multiPositions");
